...
public class DownloadItemExportServlet extends DSpaceServlet {
	...
	protected void doDSGet(Context context, HttpServletRequest request,
		...
		if(itemId != null && format != null && NumberUtils.isNumber(itemId))
		{
			...
			if(exportType != null)
			{
				...
				String processedFile = new ItemExportFormats().process(item, exportType);
	 ...
	 * @param fileContent String representing file contents
	 * @param exportType Export format (endonte, etc..)
	 * @throws IOException
	 * @throws FileNotFoundException
	 */
	private void doDownload(HttpServletResponse response, String fileContent, ExportType exportType) throws IOException,
			FileNotFoundException {
		
		ServletOutputStream servletOutputStream = response.getOutputStream();
		
		response.setContentType("text/plain");
		response.setContentLength((int)fileContent.getBytes().length);
		
		StringBuilder contentDispositionBuilder = new StringBuilder();
		contentDispositionBuilder.append("attachment; filename=\"item-export.");
		...
		servletOutputStream.write(fileContent.getBytes());
